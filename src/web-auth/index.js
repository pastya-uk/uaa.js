...
function WebAuth(options) {
  ...
  assert.check(options, { type: 'object', message: 'options parameter is not valid' }, {
	...
	_basicAuthentication: { optional: true, type: 'string', message: '_basicAuthentication option is not valid' },
  ...
  if (options.overrides) {
    assert.check(options.overrides, { type: 'object', message: 'overrides option is not valid' }, {
	  __secure_channel: { optional: true, type: 'boolean', message: '__secure_channel option is not valid' },
      __tenant: { optional: true, type: 'string', message: '__tenant option is required' },
      __token_issuer: { optional: true, type: 'string', message: '__token_issuer option is required' }
    });
  }
  /* eslint-enable */
  ...
  this.baseOptions.prefix = this.overrides.__secure_channel === false ? 'http://' : 'https://';
	
  this.baseOptions.token_issuer = (this.overrides && this.overrides.__token_issuer)
    || this.baseOptions.prefix + this.baseOptions.domain + '/';
 ...
 *
 * Only validates id_tokens signed by Demica Securoty Server using the RS256 algorithm using the 
 * public key exposed by the `/.well-known/jwks.json` endpoint. Id tokens signed with other 
 * algorithms will not be accepted.
 *
 * @method parseHash
 * @param {Object} options:
 ...
 * @param {Object} options: Link to the documentation
 * @param {Function} cb
 */
...
/**
 * Redirects to the uaa logout page
 *
 * @method logout
 * @param {Object} options: Link to the documentation
 */
